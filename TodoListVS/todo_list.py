...
def make_view(config: Config):
    ...
    tl = tk.Listbox(
        ...
        font=FONT,
        width=64,
        yscrollcommand=lambda_make_view_align0,
    ...
    pl = tk.Listbox(
        ...
        font=FONT,
        width=64,
        yscrollcommand=lambda_make_view_align1,
    ...
    def lambda_at_todo(event):
        ...
        get_confirm(
            "confirm",
            "send it to pass? \n" + s,
            lambda: click_list_todo(
                event, tlb, todo_data, (dl0, dl1, tl, pl, tlb, plb, todo_data, config)
            ),
        )
    ...
    def lambda_at_pass(event):
        ...
        s = todo_data["pass"][
            len(todo_data["pass"]) - pass_id_mapping[plb.curselection()[0]] - 1
        ]

        get_confirm(
            "confirm",
            "send it to todo? \n" + s,
            lambda: click_list_pass(
                event, plb, todo_data, (dl0, dl1, tl, pl, tlb, plb, todo_data, config)
            ),
        )
    ...
    def lambda_at_todo_list(event):
        if len(tl.curselection()) == 0:
            return
        click_to_add(
            event, tl, todo_data, (dl0, dl1, tl, pl, tlb, plb, todo_data, config)
        )
